...
for k in range(NUM_EPISODES):
    ...
    while state not in TERMINAL_STATES:
        ...
        #updates the state
        new_state = tuple(np.add(state, selected_action))

        NEW_VALID_ACTIONS = get_valid_actions(new_state)
        
        if NEW_VALID_ACTIONS == None:
            #All terminal states have state-value of zero
            ACTION_VALUES[(state, selected_action)] = (1-ALPHA)*ACTION_VALUES[(state, selected_action)] + ALPHA*(reward)

        else:
            NEW_PROBABILITES = [POLICY[(new_state,action)] for  action in NEW_VALID_ACTIONS]
            new_action = random.choices(NEW_VALID_ACTIONS, NEW_PROBABILITES)[0]

            #Update action values
            ACTION_VALUES[(state, selected_action)] = (1-ALPHA)*ACTION_VALUES[(state, selected_action)] + ALPHA*(reward + DISCOUNT_FACTOR*ACTION_VALUES[(new_state, new_action)])
